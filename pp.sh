...
# mayoría por paru, sin prompts
paru_install(){
  ensure_paru
  paru -S --needed --noconfirm --skipreview --removemake "$@"
}
...
echo -e "\n\n${blueColour}[*] Instalando paquetes (mayoría via paru)...${endColour}"
sleep 1

# Todo lo de repos + AUR aquí, SIN zscroll-git
paru_install \
...
# zscroll sin AUR (evita el error): pip en user
echo -e "\n${purpleColour}[*] Installing zscroll (pip user, no AUR)...${endColour}"
python -m pip install --user --upgrade pip >/dev/null 2>&1 || true
python -m pip install --user zscroll
...
echo -e "\n${purpleColour}[*] Cleanup tools...${endColour}"
